...
export default function AIPhotoStudio({
  ...
  const startCamera = async (deviceId?: string) => {
    ...
    try {
      ...
      setCameraError(
        err.message ||
          "Could not access camera. Please verify camera permissions are granted.",
      );
    }
  };
  ...
  const [toolMode, setToolMode] = useState<"paint" | "picker" | "polygon">(
    "paint",
  );
  ...
  const [pickedPixelColor, setPickedPixelColor] = useState<{
    hex: string;
  ...
  useEffect(() => {
    async function preload() {
      try {
        const { shades: pool } = await shadeService.getShades({
          brand: "all",
          limit: 20000,
        });
        setFullPool(pool);
  ...
  const pickerMatches = useMemo(() => {
    ...
    const findBestForBrand = (brandId: string) => {
      ...
      for (const shade of poolToUse) {
        if (!shade.brand.toLowerCase().includes(brandId.toLowerCase()))
          continue;
      ...
      const similarity = Math.max(
        0,
        Math.min(100, Math.round((1 - minDistance / 442) * 100)),
      );
    ...
    async function fetchColorMeta() {
      ...
      try {
        ...
        setColorInfo({
          ...
          complementary: [
            data.image?.bare ||
              `https://www.thecolorapi.com/id?hex=${cleanHex}&format=svg`,
          ],
  ...
  useEffect(() => {
    ...
    img.onerror = () => {
      setLoading(false);
      alert(
        "Failed to load image. If on localhost, verify connection to unsplash.",
      );
    };
  }, [selectedImage]);
  ...
  const performInteractivePaint = (x: number, y: number) => {
    ...
    for (let i = 0; i < mask.length; i++) {
      if (mask[i]) {
        ...
        currentData.data[idx + 1] = Math.floor(
          (1 - alpha) * g + alpha * paintG,
        );
        currentData.data[idx + 2] = Math.floor(
          (1 - alpha) * b + alpha * paintB,
        );
      }
    }
  ...
  const runFloodFill = (
    ...
    const startLuma = Math.max(
      1,
      0.299 * startR + 0.587 * startG + 0.114 * startB,
    );
    ...
    const checkAndPush = (
      ...
      if (
        globalDist <= tol ||
        (chromaDist <= 12 && localDist <= 10 && globalDist <= tol * 3.5)
      ) {
        visited[nIdx] = 1;
        mask[nIdx] = 1;
        queue.push(nx, ny);
  ...
  const handleSaveToCloud = async () => {
    ...
    try {
      ...
      alert(
        "Failed to save project. Ensure Firebase database permissions are valid.",
      );
    } finally {
      setIsSaving(false);
    }
              ...
              <button
                onClick={() => {
                  if (canvasRef.current) {
                    exportCanvasAsImage(
                      canvasRef.current,
                      `rainbowpaint-preview-${Date.now()}.png`,
                    );
                  }
                }}
        ...
        <div className="relative flex-grow flex items-center justify-center bg-zinc-950/5 min-h-[300px] sm:min-h-[420px] max-h-[500px] overflow-hidden">
          ...
          {isAiProcessing && (
            <div className="absolute inset-0 bg-black/40 backdrop-blur-md z-50 flex flex-col items-center justify-center text-center text-white px-4">
              ...
              <p className="text-sm font-semibold tracking-wide">
                SAM Engine detecting room surfaces...
              </p>
              <p className="text-xs text-zinc-300 mt-1">
                Applying ML-assisted wall and contour alignment.
              </p>
            </div>
          )}
                  ...
                  {polygonPoints.length > 0 && (
                    <polygon
                      points={polygonPoints
                        .map(
                          (p) =>
                            `${(p.x / canvasRef.current!.width) * 100}%,${(p.y / canvasRef.current!.height) * 100}%`,
                        )
                        .join(" ")}
                      fill={
                        activeShade
            ...
            <div className="w-full max-w-xl mx-auto relative overflow-hidden rounded-xl border border-zinc-200 bg-black flex flex-col shadow-2xl relative z-10">
              <div className="px-4 py-3 bg-white border-b border-zinc-200 flex justify-between items-center text-zinc-900">
                ...
                {availableCameras.length > 1 && (
                  <div className="relative text-zinc-900 font-sans text-[10px] z-[99]">
                    ...
                    >
                      ...
                      <ChevronDown
                        className={`w-3 h-3 text-zinc-600 shrink-0 transition-transform duration-300 ${isCameraDropdownOpen ? "rotate-180" : ""}`}
                      />
                    </button>
                ...
                <button
                  type="button"
                  onClick={stopCamera}
                  className="hover:text-gold text-zinc-600 transition-colors p-1 cursor-pointer"
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
          ...
          {!selectedImage && !isCameraActive && (
            <div className="flex flex-col items-center justify-center text-center p-8 bg-white border border-zinc-200/60 rounded-2xl w-full max-w-md shadow-xs relative">
              ...
              <div className="text-[10px] sm:text-[11px] font-sans font-bold uppercase tracking-widest text-zinc-950 mb-1">
                No Image Loaded
              </div>
              <p className="text-[11px] text-zinc-600 font-sans max-w-xs mb-5">
                Upload a photo of your own room or use your camera to capture
                your walls and visualize colors in real-time.
              </p>
            ...
            <div className="flex flex-col md:flex-row items-stretch gap-4">
              ...
              <div className="flex items-center gap-3 bg-white p-3 rounded-xl border border-zinc-200/60 shadow-xs md:w-1/3 shrink-0">
                <div
                  ...
                  style={{
                    backgroundColor: pickedPixelColor
                      ? pickedPixelColor.hex
                      : "#cca564",
                  }}
                />
                <div className="min-w-0">
                  ...
                  <span className="text-[9px] text-zinc-600 block mt-0.5 leading-none">
                    {pickedPixelColor
                      ? `R: ${pickedPixelColor.rgb.r}, G: ${pickedPixelColor.rgb.g}, B: ${pickedPixelColor.rgb.b}`
                      : "Click any spot on image to sample"}
                  </span>
                </div>
              </div>
              ...
              <div className="flex-grow min-w-0">
                ...
                {pickerMatches ? (
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    {pickerMatches.asian ? (
                      <div
                        className={`bg-white p-2.5 rounded-xl border flex items-center justify-between gap-1 transition-all duration-300 hover:-translate-y-0.5 ${activeShade?.id === pickerMatches.asian.shade.id ? "border-gold ring-1 ring-gold/30 shadow-[0_4px_10px_rgba(200,165,100,0.1)]" : "border-zinc-200/80 hover:border-gold/30"}`}
                      >
                        <button
                          type="button"
                          onClick={() =>
                            onSelectShade &&
                            onSelectShade(pickerMatches.asian!.shade)
                          }
                        ...
                        >
                          <div
                            ...
                            style={{
                              backgroundColor: pickerMatches.asian.shade.hex,
                            }}
                          />
                          <div className="min-w-0 flex-grow leading-tight">
                            ...
                            <span className="text-[10px] sm:text-[11px] font-sans font-bold uppercase tracking-widest text-zinc-800 block truncate">
                              {pickerMatches.asian.shade.name}
                            </span>
                            <span className="text-[7.5px] font-mono text-zinc-600 block">
                              {pickerMatches.asian.shade.shadeCode}
                            </span>
                          </div>
                        </button>
                    ...
                    {pickerMatches.berger ? (
                      <div
                        className={`bg-white p-2.5 rounded-xl border flex items-center justify-between gap-1 transition-all duration-300 hover:-translate-y-0.5 ${activeShade?.id === pickerMatches.berger.shade.id ? "border-gold ring-1 ring-gold/30 shadow-[0_4px_10px_rgba(200,165,100,0.1)]" : "border-zinc-200/80 hover:border-gold/30"}`}
                      >
                        <button
                          type="button"
                          onClick={() =>
                            onSelectShade &&
                            onSelectShade(pickerMatches.berger!.shade)
                          }
                          className="flex-1 flex items-center gap-2.5 min-w-0 text-left"
                        >
                          <div
                            className="w-8 h-8 rounded-md flex-shrink-0 border"
                            style={{
                              backgroundColor: pickerMatches.berger.shade.hex,
                            }}
                          />
                          <div className="min-w-0 flex-grow leading-tight">
                            <span className="text-[7.5px] font-display font-bold uppercase text-zinc-600 tracking-wider block">
                            ...
                            <span className="text-[10px] sm:text-[11px] font-sans font-bold uppercase tracking-widest text-zinc-800 block truncate">
                              {pickerMatches.berger.shade.name}
                            </span>
                            <span className="text-[7.5px] font-mono text-zinc-600 block">
                              {pickerMatches.berger.shade.shadeCode}
                            </span>
                          </div>
                        </button>
                        <div className="flex flex-col items-end gap-1.5 shrink-0">
                    ...
                    {pickerMatches.mrf ? (
                      <div
                        className={`bg-white p-2.5 rounded-xl border flex items-center justify-between gap-1 transition-all duration-300 hover:-translate-y-0.5 ${activeShade?.id === pickerMatches.mrf.shade.id ? "border-gold ring-1 ring-gold/30 shadow-[0_4px_10px_rgba(200,165,100,0.1)]" : "border-zinc-200/80 hover:border-gold/30"}`}
                      >
                        <button
                          type="button"
                          onClick={() =>
                            onSelectShade &&
                            onSelectShade(pickerMatches.mrf!.shade)
                          }
                          className="flex-1 flex items-center gap-2.5 min-w-0 text-left"
                        >
                          <div
                            className="w-8 h-8 rounded-md flex-shrink-0 border"
                            style={{
                              backgroundColor: pickerMatches.mrf.shade.hex,
                            }}
                          />
                          <div className="min-w-0 flex-grow leading-tight">
                            <span className="text-[7.5px] font-display font-bold uppercase text-zinc-600 tracking-wider block">
                            ...
                            <span className="text-[10px] sm:text-[11px] font-sans font-bold uppercase tracking-widest text-zinc-800 block truncate">
                              {pickerMatches.mrf.shade.name}
                            </span>
                            <span className="text-[7.5px] font-mono text-zinc-600 block">
                              {pickerMatches.mrf.shade.shadeCode}
                            </span>
                          </div>
                        </button>
                        <div className="flex flex-col items-end gap-1.5 shrink-0">
                  ...
                  <div className="bg-white border border-dashed border-zinc-200 p-3.5 rounded-xl flex items-center justify-center text-center h-[46px]">
                    <p className="text-[10px] text-zinc-600 italic font-sans">
                      No points sampled yet. Click anywhere on your room canvas
                      above to find matches.
                    </p>
                  </div>
                )}
              </div>
